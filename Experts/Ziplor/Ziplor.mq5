...
#include "..\..\Include\Ziplor\ZiplorEnums.mqh"
#include "..\..\Include\Ziplor\ZiplorStructures.mqh"
#include "..\..\Include\Ziplor\ZiplorSMCEngine.mqh"
#include "..\..\Include\Ziplor\ZiplorDashboard.mqh"
   ...
   if(g_recovery.recovered >= g_recovery.loss_to_recover)
     {
      Print("RECOVERY COMPLETE: Recovered ", DoubleToString(g_recovery.recovered, 2),
            " in ", g_recovery.recovery_trades, " trades");
      g_recovery.state = RECOVERY_OFF;
      g_recovery.loss_to_recover = 0;
      g_recovery.recovered = 0;
      g_recovery.recovery_trades = 0;
      g_am_state.current_risk_pct = g_recovery.original_risk_pct;
     }
   //--- Check if max recovery trades exceeded
   else if(g_recovery.recovery_trades >= g_recovery.max_recovery_trades)
     {
      Print("RECOVERY FAILED: Max trades reached. Remaining loss: ",
            DoubleToString(g_recovery.loss_to_recover - g_recovery.recovered, 2));
      g_recovery.state = RECOVERY_OFF;
      g_am_state.current_risk_pct = g_am_state.base_risk_pct;
     }
  }
...
//+------------------------------------------------------------------+
//| Check for daily/weekly P&L resets                                |
//+------------------------------------------------------------------+
void CheckDailyWeeklyReset()
  {
   MqlDateTime dt_current, dt_last_daily, dt_last_weekly;
   TimeToStruct(TimeCurrent(), dt_current);
   TimeToStruct(g_last_daily_reset, dt_last_daily);
   TimeToStruct(g_last_weekly_reset, dt_last_weekly);

   //--- Daily reset at midnight
   if(dt_current.day_of_year != dt_last_daily.day_of_year ||
      dt_current.year != dt_last_daily.year)
     {
      g_daily_start_balance = g_account.Balance();
      g_last_daily_reset = TimeCurrent();
     }
   ...
   if(dt_current.day_of_week == 1 && dt_last_weekly.day_of_week != 1)
     {
      g_weekly_start_balance = g_account.Balance();
      g_last_weekly_reset = TimeCurrent();
     }
...
void UpdateDashboard()
  {
   ...
   //--- Account
   data.balance = g_account.Balance();
   data.equity = g_account.Equity();
   data.margin_used = g_account.Margin();
   data.free_margin = g_account.FreeMargin();
   data.daily_pnl = g_account.Balance() - g_daily_start_balance;
   data.weekly_pnl = g_account.Balance() - g_weekly_start_balance;

   //--- Positions
   data.open_positions = CountOpenPositions();
   data.total_lots = 0;
   data.floating_pnl = 0;
   for(int i = PositionsTotal() - 1; i >= 0; i--)
     {
      if(g_position.SelectByIndex(i))
        {
         if(g_position.Symbol() == _Symbol && g_position.Magic() == InpMagicNumber)
           {
            data.total_lots += g_position.Volume();
            data.floating_pnl += g_position.Profit() + g_position.Swap() + g_position.Commission();
           }
        }
     }

   //--- Market structure
   data.htf_structure = g_htf_structure;
   data.mtf_structure = g_mtf_structure;
   data.ltf_structure = g_ltf_structure;
   data.exec_structure = g_exec_structure;

   //--- Signal
   data.current_signal = g_current_signal;

   //--- Anti-martingale
   data.am_state = g_am_state;

   //--- Recovery
   data.recovery = g_recovery;

   //--- Session
   data.current_session = g_current_session;
